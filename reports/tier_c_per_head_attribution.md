...
3. **Cumulative top-k not finished.** The cumulative analysis — does concatenating the top-k heads' vectors and refitting Ridge climb toward the full-layer R²? — was implemented but hit slow Ridge on concatenated D up to 200 k features. A sample-space ridge (`(X X^T + αI)^{-1} y`) would solve that in minutes. That's the quickest follow-up.
